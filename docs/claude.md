...
*Estimates based on Groq's pricing as of mid-2026. Run once, never again.*  
*Alternatively: use Claude Haiku for Pass 1 at comparable cost with higher extraction accuracy.*
...
- The core value proposition is that PlotWise builds its knowledge *from the book itself*, not from the internet.
...
*This document is the single source of truth for PlotWise's architecture. Update it when design decisions change. Never let the code and this document diverge.*